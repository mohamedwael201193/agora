...
import { AgoraClient, generateCommitment, randomSaltHex, saveSalt } from "@/lib/agoraClient";
...
export function BetTicket({ market, open, onOpenChange }: BetTicketProps) {
  ...
  const handlePlaceBet = async () => {
    console.log('[BetTicket] handlePlaceBet called');
    console.log('[BetTicket] market:', market);
    console.log('[BetTicket] market.chainId:', market.chainId);
    console.log('[BetTicket] side:', side);
    console.log('[BetTicket] amount:', amount);
    
    if (!validateBet()) {
      console.log('[BetTicket] Validation failed:', error);
    ...
    try {
      // Generate cryptographic commitment for the bet
      // Generate random salt
      const saltHex = randomSaltHex();
      
      // Generate commitment hash from choice + salt
      const commitmentHex = await generateCommitment(side, saltHex);
      
      // Save salt locally for later reveal
      const chainIdForSalt = market.chainId || market.id;
      saveSalt(chainIdForSalt, saltHex);

      // Submit commitment to Linera blockchain
      const result = await agoraClient.commit(market.chainId || '', commitmentHex);
      ...
      addNotification({
        type: "bet_placed",
        title: "Bet Committed to Blockchain",
        message: `${side} on "${market.question}" - commitment recorded on Linera`,
      ...
      toast.success("Bet committed!", {
        description: `${side} position committed on blockchain. You can reveal it during the reveal phase.`,
        duration: 5000,
      });
      ...
      // Check if error is due to already having committed
      const isAlreadyCommitted = errorMessage.includes('Already committed') || 
                                  errorMessage.includes('already committed');
      
      setError(errorMessage);
      
      if (isAlreadyCommitted) {
        toast.error("Already Bet on This Market", { 
          description: "You have already placed a bet on this market. Each wallet can only bet once per market.",
          duration: 6000,
        });
      } else {
        toast.error("Bet failed", { 
          description: `Failed to commit bet: ${errorMessage}`,
          duration: 5000,
        });
      }
    }

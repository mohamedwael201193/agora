            ...
            <ScrollArea className="flex-1 p-4">
              <div className="space-y-6">
                ...
                <div className="space-y-3 pt-4 border-t border-border">
                  ...
                  <div className="space-y-2">
                    {(['local', 'conway'] as const).map((mode) => (
                      <button
                        key={mode}
                        onClick={() => {
                          switchNetwork(mode);
                          toast.success(`Switched to ${mode === 'conway' ? 'Conway Testnet' : 'Local Network'}`);
                        }}
                        className={`w-full p-3 rounded-lg border text-left transition-all ${
                          network === mode
                            ? "bg-green-500/10 border-green-500/50 text-text-primary"
                            : "bg-surface border-border hover:border-border text-text-secondary"
                        }`}
                      >
                        <div className="font-medium capitalize">
                          {mode === 'local' ? 'Local Development' : 'Conway Testnet'}
                        </div>
                        <div className="text-xs text-text-muted mt-1">
                          {mode === 'local' && 'Local 4-validator network on localhost:8080'}
                          {mode === 'conway' && 'Public testnet with real validators'}
                        </div>
                      </button>
                    ))}
                  </div>

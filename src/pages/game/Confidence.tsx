import { OnChainLog } from "@/components/game/OnChainLog";
import { ProofPanel } from "@/components/game/ProofPanel";
...
export default function Confidence() {
  ...
  // Initialize Linera client and test connection
  useEffect(() => {
    const initializeLinera = async () => {
      try {
        await lineraClient.testConnection();
      } catch (error) {
        console.warn("Linera client initialization failed, using sim mode");
      }
    };
  ...
  const handleSubmitPrediction = async (side: "YES" | "NO", prob: number) => {
    ...
    try {
      // Write to chain if in live mode
      await lineraClient.writeGameRound({
        roundId: `round_${currentGame.roundNumber + 1}_${Date.now()}`,
        userPseudo: `Predictor${Math.floor(Math.random() * 999)
          .toString()
          .padStart(3, "0")}`,
        probability: prob,
        outcome: side,
      ...
      // Update store on successful write
      recordChainWrite();
      setChainError(null);
    } catch (error) {
      console.warn("Chain write failed:", error);
      setChainError(error instanceof Error ? error.message : "Write failed");
      // Continue with game even if chain write fails
    }
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      ...
      <div className="mb-8 space-y-6">
        ...
        >
          <div className="flex items-center gap-4 p-3 glass-surface rounded-lg border">
            ...
            <div className="flex rounded-md overflow-hidden border border-border/50">
              <button
                onClick={() => setLineraMode("live")}
                className={`px-4 py-2 text-sm font-medium transition-all ${
                  lineraMode === "live"
                    ? "bg-success text-white"
                    : "bg-surface hover:bg-surface-elevated text-text-muted"
                }`}
              >
                🟢 Live (Conway)
              </button>
              <button
                onClick={() => setLineraMode("sim")}
                className={`px-4 py-2 text-sm font-medium transition-all ${
                  lineraMode === "sim"
        ...
        {/* Proof Panel - Compact version always visible */}
        <ProofPanel compact className="justify-center" />
      </div>
      ...
      {/* Judge Verification Sections */}
      <div className="mt-12 space-y-8">
        {/* Full Proof Panel for Judges */}
        <motion.div

...
import { formatChainId, formatTransactionHash, gameBridge, GameStats } from "@/services/GameBridge";
import { motion } from "framer-motion";
import { Activity, Clock, DollarSign, Link as LinkIcon, Target, Timer, Zap } from "lucide-react";
...
export default function ChronoEchoes() {
  ...
  const [isPlacingBet, setIsPlacingBet] = useState<number | null>(null);
  const { toast } = useToast();

  // Initialize game bridge and load stats
  useEffect(() => {
    const initializeGame = async () => {
      try {
        await gameBridge.initialize();
        const stats = await gameBridge.getGameStats();
        setGameStats(stats);
      } catch (error) {
        console.error("Failed to initialize game:", error);
        toast({
          title: "Initialization Failed",
          description: "Could not connect to Linera counter application",
          variant: "destructive",
        });
      }
    };
  ...
  // Handle placing a prediction bet
  const handlePlaceBet = async (eventId: number, prediction: "YES" | "NO") => {
    setIsPlacingBet(eventId);
    ...
    try {
      ...
      const result = await gameBridge.onGameRoundComplete({
        roundId: `chrono_${eventId}_${Date.now()}`,
        userPseudo: `Player${Math.floor(Math.random() * 999)}`,
        probability,
        outcome: prediction,
        brierScore: 0, // Will be calculated after resolution
      });

      if (result.success) {
        // Refresh game stats
        const newStats = await gameBridge.getGameStats();
        setGameStats(newStats);

        toast({
          title: "Prediction Recorded!",
          description: `Counter incremented to ${result.newValue}. Tx: ${formatTransactionHash(result.transactionHash || '')}`,
        });
      } else {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      >
        ...
        {/* Linera Counter Status */}
        {gameStats && (
          <Card className="p-6 glass-surface mb-6 border-purple-deep/30">
            <h3 className="font-semibold mb-4 flex items-center gap-2">
              <Activity className="w-5 h-5 text-purple-deep" />
              Linera Counter Status
              <Badge variant={gameStats.chainConnected ? "default" : "destructive"} className="ml-2">
                {gameStats.mode === "live" ? "LIVE" : "SIM"}
              </Badge>
            </h3>
            <div className="grid md:grid-cols-3 gap-4 text-sm">
              <div>
                <span className="text-text-muted">Total Predictions:</span>
                <div className="font-mono text-lg font-bold text-purple-deep">{gameStats.totalRounds}</div>
              </div>
              <div>
                <span className="text-text-muted">Counter Value:</span>
                <div className="font-mono text-lg font-bold text-orange-primary">{gameStats.currentValue}</div>
              </div>
              <div>
                <span className="text-text-muted">Chain Status:</span>
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${gameStats.chainConnected ? 'bg-success' : 'bg-danger'}`} />
                  <span className={gameStats.chainConnected ? 'text-success' : 'text-danger'}>
                    {gameStats.chainConnected ? 'Connected' : 'Disconnected'}
                  </span>
                </div>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-gray-700/20 flex justify-between items-center">
              <Button
                variant="outline"
                size="sm"
                onClick={() => window.open(gameBridge.getGraphQLPlaygroundURL(), '_blank')}
              ...
              <div className="text-xs text-text-muted">
                Chain: {formatChainId(gameStats.chainConnected ? 'connected' : 'local-dev')}
              </div>
            </div>
          </Card>
        )}
              ...
              <div className="grid md:grid-cols-[1fr_auto] gap-6">
                ...
                <div className="flex flex-col items-center justify-center gap-4 md:min-w-[200px]">
                  ...
                  <div className="flex gap-2 w-full">
                    <Button 
                      className="flex-1 bg-gradient-to-r from-success/80 to-success hover:opacity-90"
                      onClick={() => handlePlaceBet(event.id, "YES")}
                      disabled={isPlacingBet === event.id}
                    >
                      {isPlacingBet === event.id ? "Recording..." : "YES"}
                    </Button>
                    <Button 
                      className="flex-1 bg-gradient-to-r from-danger/80 to-danger hover:opacity-90"
                      onClick={() => handlePlaceBet(event.id, "NO")}
                      disabled={isPlacingBet === event.id}
                    >
                      {isPlacingBet === event.id ? "Recording..." : "NO"}
                    </Button>
                  </div>
                </div>
              </div>
